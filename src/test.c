...
START_TEST(exp_tests) { s21_generic_ld_test(_i, s21_exp, exp); }
END_TEST

START_TEST(ceil_tests) { s21_generic_ld_test(_i, s21_ceil, ceil); }
END_TEST

START_TEST(floor_tests) { s21_generic_ld_test(_i, s21_floor, floor); }
END_TEST
...
START_TEST(tan_tests) { s21_generic_ld_test(_i, s21_tan, tan); }
END_TEST

// START_TEST(fmod_tests) { s21_generic_ld_test(_i, s21_fmod, fmod); }
// END_TEST

START_TEST(asin_tests) { s21_generic_ld_test(_i, s21_asin, asin); }
END_TEST

START_TEST(acos_tests) { s21_generic_ld_test(_i, s21_acos, acos); }
END_TEST
...
int main() {
  ...
  int tc_double_2p_len = sizeof(tc_double_2p) / sizeof(tc_double_2p[0]);
  Suite *s21_math_tests[] = {
      ...
      s21_generic_ts(pow_tests, tc_double_2p_len / 2, S21_TITLE_POW),
      ...
      // s21_generic_ts(fmod_tests, tc_double_len, S21_TITLE_FMOD),
      s21_generic_ts(asin_tests, tc_double_len, S21_TITLE_ASIN),
      s21_generic_ts(acos_tests, tc_double_len, S21_TITLE_ACOS), NULL};
...
void s21_generic_ld_test(int idx, long double (*s21_func)(double),
  ...
  int b_isinf = isinf(b);
  if (a_isnan || b_isnan) {
    ck_assert_int_eq(a_isnan, b_isnan);
  } else if (a_isinf || b_isinf) {
    ck_assert_int_eq(a_isinf, b_isinf);
  } else {
